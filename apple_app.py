...
with col_b1:
    if st.button("➕ လွတ်ငြိမ်းချမ်းသာပေးသည့်ရက်စွဲထည့်ပါ"): st.session_state.lucky_draws.append({"date": datetime.now().date(), "type": "ကျခံရမည့်ပြစ်ဒဏ်၏ ၁/၄"})
with col_b2:
...
if st.button("အတည်ပြုပါသည်။ တွက်ချက်မည်။", use_container_width=True):
    ...
    while curr <= f2_e:
        ...
        if d_str in lucky_data_map:
            ...
            total += added
            breakdown_notes.append(f"🎡 **{d_str}** - {l_type} အရ ({round(added, 2)} ရက်) ပေါင်းထည့်ခဲ့သည်။ (ထိုစဉ်က ကျန်ရှိသော ပြစ်ဒဏ်: {round(current_rem, 2)})")
        ...
        if curr >= stage_info['bonus_start']:
            if stage_info['days_counter'] == 0:
                ...
                total += b_amt
                breakdown_notes.append(f"📅 **{d_str}** - ၃ လပတ်အပိုဆု ကွာ (Stage {stage_info['stage']}) အဖြစ် **{b_amt} ရက်** ရရှိသည်။")
    ...
    if total < target:
        ...
        while curr <= f3_e:
            ...
            if d_str in lucky_data_map:
                l_type = lucky_data_map[d_str]
                current_rem = max(0, target - total)
                added = (target/4 if l_type=="ကျခံရမည့်ပြစ်ဒဏ်၏ ၁/၄" else target/6 if l_type=="ကျခံရမည့်ပြစ်ဒဏ်၏ ၁/၆" else current_rem/4 if l_type=="ကျခံရန်ကျန်ရှိသောပြစ်ဒဏ်၏ ၁/၄" else current_rem/6)
                total += added
    ...
    with st.container():
        ...
        > * **ကျခံရန်ကျန်ရှိသောပြစ်ဒဏ်၏ ၁/၄ သို့မဟုတ် ၁/၆:** တွက်ချက်သည့်ရက်စွဲတွင် (Target {target} - ထိုနေ့အထိရရှိထားသော ‌လျှော့ရက်) ကို ရှာဖွေပြီး ၄ သို့မဟုတ် ၆ နှင့် စားခြင်းဖြစ်ပါသည်။
        > * **Stage Bonus (၃ လပတ်):** ဒုတိယနေရာစတင်ချိန်မှ ရက်ပေါင်း ၉၀ တိုင်းတွင် Stage အလိုက် (၁၁၊ ၂၀၊ ၂၉) ရက် ပေါင်းထည့်ပါသည်။ (လက်ရှိ Stage {stage_info['stage']} အထိ တွက်ချက်ပြီး)
        """)
